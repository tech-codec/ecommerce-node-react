...
const DeleteRoleModal = ({ show, currentRoleId, onDelete, onClose }) => {

  const {theme} = useTheme()

  const handleSubmit = () => {
    onDelete(currentRoleId);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 px-5 flex justify-center items-center">
      <div className={`${theme === 'dark' ? 'bg-gray-700' : 'bg-white'} w-550px p-4 rounded`}>
        <div className='flex items-center flex-col'>
            ...
            <p className='text-gray-500 mb-6 text-lg md:text-xl text-center'>Etes-vous sûr de vouloir supprimer <br/> ce rôle ?</p>
        </div>
